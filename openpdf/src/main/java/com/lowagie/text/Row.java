...
/**
 * A <CODE>Row</CODE> is part of a <CODE>Table</CODE> and contains some <CODE>Cells</CODE>.
 * <p>
 * All <CODE>Row</CODE>s are constructed by a <CODE>Table</CODE>-object. You don't have to construct any
 * <CODE>Row</CODE> yourself. In fact you can't construct a <CODE>Row</CODE> outside the package.
 * <p>
 * Since a <CODE>Cell</CODE> can span several rows and/or columns a row can contain reserved space without any content.
 *
 * @see Element
 * @see Cell
 * @see Table
...
public class Row implements Element, WithHorizontalAlignment {
    ...
    /**
     * Returns a <CODE>Row</CODE> that is a copy of this <CODE>Row</CODE> in which a certain column has been deleted.
     *
    ...
    void deleteColumn(int column) {
        if ((column >= columns) || (column < 0)) {
            throw new IndexOutOfBoundsException(
                    MessageLocalization.getComposedMessage("getcell.at.illegal.index.1", column));
        }
        columns--;
     ...
     * @return the column position the <CODE>Cell</CODE> was added, or <CODE>-1</CODE> if the <CODE>element</CODE>
     * couldn't be added.
     */
     ...
     * @return the column position the <CODE>Cell</CODE> was added, or <CODE>-1</CODE> if the <CODE>Cell</CODE> couldn't
     * be added.
     */
    int addElement(TableRectangle element, int column) {
        ...
        if ((column < 0) || (column > columns)) {
            throw new IndexOutOfBoundsException(
                    MessageLocalization.getComposedMessage("addcell.illegal.column.argument"));
        }
        if (!((getObjectID(element) == CELL) || (getObjectID(element) == TABLE))) {
            throw new IllegalArgumentException(
                    MessageLocalization.getComposedMessage("addcell.only.cells.or.tables.allowed"));
        }
    ...
    void setElement(TableRectangle aElement, int column) {
        if (reserved[column]) {
            throw new IllegalArgumentException(
                    MessageLocalization.getComposedMessage("setelement.position.already.taken"));
        }
    ...
    boolean reserve(int column, int size) {
        if ((column < 0) || ((column + size) > columns)) {
            throw new IndexOutOfBoundsException(
                    MessageLocalization.getComposedMessage("reserve.incorrect.column.size"));
        }
    ...
    /**
     * Returns true/false when this position in the <CODE>Row</CODE> has been reserved, either filled or through a
     * colspan of an Element.
     *
    ...
    int getElementID(int column) {
        if (cells[column] == null) {
            return NULL;
        } else if (cells[column] instanceof Cell) {
            return CELL;
        } else if (cells[column] instanceof Table) {
            return TABLE;
        }
     ...
     * @return the <CODE>Cell</CODE>,<CODE>Table</CODE> or <VAR>Object</VAR> if the column was reserved or null if
     * empty.
     */
    public TableRectangle getCell(int column) {
        if ((column < 0) || (column > columns)) {
            throw new IndexOutOfBoundsException(
                    MessageLocalization.getComposedMessage("getcell.at.illegal.index.1.max.is.2",
                            String.valueOf(column), String.valueOf(columns)));
        }
        return cells[column];
    }
