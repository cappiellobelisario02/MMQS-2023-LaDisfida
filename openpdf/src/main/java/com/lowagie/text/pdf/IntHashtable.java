 ...
 */
 ...
 public class IntHashtable {
     ...
     public IntHashtable(int initialCapacity, float loadFactor) {
         ...
         if (initialCapacity < 0) {
             throw new IllegalArgumentException(
                     MessageLocalization.getComposedMessage("illegal.capacity.1", initialCapacity));
         }
         if (loadFactor <= 0) {
             throw new IllegalArgumentException(
                     MessageLocalization.getComposedMessage("illegal.load.1", String.valueOf(loadFactor)));
         }
         if (initialCapacity == 0) {
             initialCapacity = 1;
         }
     ...
     /***
      * <p>Constructs a new hashtable with the same mappings as the specified
      * hashtable.</p>
      *
      * @param original the hashtable to be copied.
      */
     public IntHashtable(IntHashtable original) {
        this.table = new Entry[original.table.length];
        this.loadFactor = original.loadFactor;
        this.threshold = original.threshold;
        this.count = original.count;

        for (int i = 0; i < original.table.length; i++) {
            if (original.table[i] != null) {
                this.table[i] = new Entry(original.table[i]);
            }
        }
     }
 ...
 // extra methods by Paulo Soares:
     ...
     static class Entry {
         ...
         protected Entry(Entry original) {
             ...
             this.next = original.next != null ? new Entry(original.next) : null;
         }
