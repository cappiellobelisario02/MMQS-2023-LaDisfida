 ...
 * dynamically by users of viewer applications. In iText they are referenced as layers.
 *
...
public class PdfLayer extends PdfDictionary implements PdfOCG {
    ...
    /**
     * Sets the <CODE>PdfIndirectReference</CODE> that represents this layer. This can only be done from PdfStamperImp.
     *
     ...
     *
     * @param lang      a language string which specifies a language and possibly a locale (for example, <B>es-MX</B>
     *                  represents Mexican Spanish)
     * @param preferred used by viewer applications when there is a partial match but no exact match between the system
     *                  language and the language strings in all usage dictionaries
     */
    ...
    /**
     * Specifies the recommended state for content in this group when the document (or part of it) is saved by a viewer
     * application to a format that does not support optional content (for example, an earlier version of PDF or a
     * raster image format).
     *
     ...
     *
     * @param min the minimum recommended magnification factors at which the group should be ON. A negative value will
     *            set the default to 0
     * @param max the maximum recommended magnification factor at which the group should be ON. A negative value will
     *            set the largest possible magnification supported by the viewer application
     */
    ...
    /**
     * Sets the visibility of the layer in Acrobat's layer panel. If <CODE>false</CODE> the layer cannot be directly
     * manipulated by the user. Note that any children layers will also be absent from the panel.
     *
