...
class TrueTypeFontSubSet {
    ...
    private int fullFontSize;
    private int tablesUsed;
    private static final String TABLE_DOES_NOT_EXIST_MESSAGE = "table.1.does.not.exist.in.2";
    ...
    protected void assembleFont() throws IOException {
        determineTableNames();
        calculateFullFontSize();
        initializeFontOutput();
        writeTableDirectory();
        writeTableContents();
    }

    private void determineTableNames() {
        if (includeExtras) {
    ...
    private void initializeFontOutput() {
    ...
    private void writeTableDirectory() {
        ...
        for (String name : tableNames) {
            ...
            if (tableLocation != null) {
                ...
                writeTableRecord(name, tableLocation, ref);
                ref += (tableLocation[TABLE_LENGTH] + 3) & (~3);
            }
        }
    }
    ...
    private void writeTableRecord(String name, int[] tableLocation, int ref) {
        ...
        writeFontInt(ref);
        writeFontInt(tableLocation[TABLE_LENGTH]);
    }

    private void writeTableContents() throws IOException {
        for (String name : tableNames) {
            int[] tableLocation = tableDirectory.get(name);
            if (tableLocation != null) {
                if (name.equals("glyf")) {
    ...
    protected void readLoca() throws IOException, DocumentException {
        ...
        if (tableLocation == null) {
            throw new DocumentException(
                    MessageLocalization.getComposedMessage(TABLE_DOES_NOT_EXIST_MESSAGE, "head", fileName));
        }
        ...
        if (tableLocation == null) {
            throw new DocumentException(
                    MessageLocalization.getComposedMessage(TABLE_DOES_NOT_EXIST_MESSAGE, "loca", fileName));
        }
        rf.seek(tableLocation[TABLE_OFFSET]);
        if (locaShortTable) {
    ...
    protected void flatGlyphs() throws IOException, DocumentException {
        ...
        if (tableLocation == null) {
            throw new DocumentException(
                    MessageLocalization.getComposedMessage(TABLE_DOES_NOT_EXIST_MESSAGE, "glyf", fileName));
        }
        Integer glyph0 = 0;
