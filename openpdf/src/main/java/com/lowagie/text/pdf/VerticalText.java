...
public class VerticalText {
     ...
     *
     * @param text the place where the text will be written to. Can be a template.
     */
    ...
    public int go(boolean simulate) {
        PdfContentByte graphics = validateAndPrepareGraphics(simulate);
        boolean dirty = false;
        int status = 0;
        ...
        while (true) {
            ...
            processLine(simulate, line);
            --maxLines;
            startX -= leading;
        }

        finalizeGraphics(dirty, graphics);
        return status;
    }

    private PdfContentByte validateAndPrepareGraphics(boolean simulate) {
        if (text != null) {
            return text.getDuplicate();
        } else if (!simulate) {
            throw new NullPointerException(MessageLocalization.getComposedMessage(
                    "verticaltext.go.with.simulate.eq.eq.false.and.text.eq.eq.null"));
        }
        return null;
    }
    ...
    private int determineStatus() {
        if (maxLines <= 0) {
            return NO_MORE_COLUMN | (chunks.isEmpty() ? NO_MORE_TEXT : 0);
        }
        if (chunks.isEmpty()) {
            return NO_MORE_TEXT;
        }
    ...
    private void processLine(boolean simulate, PdfLine line) {
        shortenChunkArray();
        if (!simulate) {
    ...
    private void finalizeGraphics(boolean dirty, PdfContentByte graphics) {
        if (dirty) {
            text.endText();
            text.add(graphics);
        }
    ...
    /**
     * Gets the X coordinate where the next line will be written. This value will change after each call to
     * <code>go()</code>.
     *
    ...
    /**
     * Gets the maximum number of available lines. This value will change after each call to <code>go()</code>.
     *
