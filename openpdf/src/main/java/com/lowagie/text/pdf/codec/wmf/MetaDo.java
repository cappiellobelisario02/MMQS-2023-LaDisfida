...
public class MetaDo {
    ...
    private void initializeState() throws IOException {
        ...
        bottom = in.readShort();
        inch = in.readWord();
        state.setScalingX((float) (right - left) / inch * 72f);
        state.setScalingY((float) (bottom - top) / inch * 72f);
        state.setOffsetWx(left);
        state.setOffsetWy(top);
        state.setExtentWx(right - left);
        state.setExtentWy(bottom - top);
        in.readInt(); // Skip reserved integer
        in.readWord(); // Skip reserved word
        in.skip(18); // Skip reserved bytes
        cb.setLineCap(1);
        cb.setLineJoin(1);
    }

    private void processMetaCommands() throws IOException {
        while (true) {
    ...
    private void processMetaFunction(int function) throws IOException {
        switch (function) {
            ...
            case META_SELECTOBJECT:
                state.selectMetaObject(in.readWord(), cb);
                break;
            case META_DELETEOBJECT:
                state.deleteMetaObject(in.readWord());
                break;
            case META_SAVEDC:
                state.saveState(cb);
                break;
            case META_RESTOREDC:
                state.restoreState(in.readShort(), cb);
                break;
            ...
            case META_SETWINDOWEXT:
                ...
                break;
            case META_MOVETO:
                state.setCurrentPoint(new Point(in.readShort(), in.readShort()));
                break;
            case META_LINETO:
                drawLine();
                break;
    ...
    private void drawPolygon() throws IOException {
    ...
    private void drawPolypolygon() throws IOException {
        if (isNullStrokeFill(false)) {
            return;
        }
    ...
    private void drawEllipse() throws IOException {
    ...
    private void drawPie() throws IOException {
        if (isNullStrokeFill(state.getLineNeutral())) {
            return;
        }
        float yend = state.transformY(in.readShort());
    ...
    private void drawChord() throws IOException {
        if (isNullStrokeFill(state.getLineNeutral())) {
            return;
        }
        float yend = state.transformY(in.readShort());
    ...
    private void drawRoundRect() throws IOException {
        if (isNullStrokeFill(true)) {
            return;
        }
    ...
    private void intersectClipRect() throws IOException {
        float b = state.transformY(in.readShort());
        float r = state.transformX(in.readShort());
        float t = state.transformY(in.readShort());
        float l = state.transformX(in.readShort());
    ...
    private String readText() throws IOException {
        ...
        return decodeText(text, k);
    }

    private String decodeText(byte[] text, int length) {
        try {
            return new String(text, 0, length, "Cp1252");
        } catch (UnsupportedEncodingException e) {
            return new String(text, 0, length);
        }
    }

    private void drawPixel() throws IOException {
