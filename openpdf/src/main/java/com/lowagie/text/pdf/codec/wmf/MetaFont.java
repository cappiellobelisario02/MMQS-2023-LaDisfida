...
public class MetaFont extends MetaObject {
    ...
    public BaseFont getFont() {
        ...
        font = createFontUsingFontFactory();
        if (font != null) {
            return font;
        }

        font = createFontBasedOnFaceName();
        return font;
    }

    private BaseFont createFontUsingFontFactory() {
        Font ff2 = FontFactory.getFont(faceName, BaseFont.CP1252, true, 10,
                ((italic != 0) ? Font.ITALIC : 0) | ((bold != 0) ? Font.BOLD : 0));
        return ff2.getBaseFont();
    }

    private BaseFont createFontBasedOnFaceName() {
    ...
    private String resolveFontName() {
        if (faceName.contains("courier") || faceName.contains("terminal")
            ...
            return fontNames[MARKER_COURIER + italic + bold];
        } else if (faceName.contains("ms sans serif") || faceName.contains("arial")
                || faceName.contains("system")) {
            return fontNames[MARKER_HELVETICA + italic + bold];
        } else if (faceName.contains("arial black")) {
            return fontNames[MARKER_HELVETICA + italic + MARKER_BOLD];
        } else if (faceName.contains("times") || faceName.contains("ms serif")
                || faceName.contains("roman")) {
            return fontNames[MARKER_TIMES + italic + bold];
        } else if (faceName.contains("symbol")) {
            return fontNames[MARKER_SYMBOL];
        } else {
            return determineFontNameFromFamily();
        }
    }

    private String determineFontNameFromFamily() {
        ...
        switch (family) {
            ...
            default:
                return pitch == FIXED_PITCH ?
                        fontNames[MARKER_COURIER + italic + bold] :
                        fontNames[MARKER_HELVETICA + italic + bold];
        }
    }
