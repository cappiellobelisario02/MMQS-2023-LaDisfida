 ...
 * can be traversed in sorted order, partial matches (wildcard) can be implemented, retrieval of all keys within a given
 * distance from the target, etc. The storage requirements are higher than a binary tree but a lot less than a trie.
 * Performance is comparable with a hash table, sometimes it outperforms a hash function (most of the time can determine
 * a miss faster than a hash).</p>
 *
 ...
 * Pretty limited!. It can be extended to a general map by using the string representation of an object and using the
 * char value as an index to an array that contains the object values.</p>
 *
...
public class TernaryTree implements Serializable {
    ...
    /**
     * We use 4 arrays to represent a node. I guess I should have created a proper node class, but somehow Knuth's
     * pascal code made me forget we now have a portable language with virtual memory management and automatic garbage
    ...
    /**
     * Branches are initially compressed, needing one node per key plus the size of the string key. They are
     * decompressed as needed when another key with same prefix is inserted. This saves a lot of space, specially for
     * long keys.
     *
    ...
    /**
     * Each node stores a character (splitchar) which is part of some key(s). In a compressed branch (one that only
     * contain a single string key) the trailer of the key which is not already in nodes is stored  externally in the kv
     * array. As items are inserted, key substrings decrease. Some substrings may completely  disappear when the whole
     * branch is totally decompressed. The tree is traversed to find the key substrings actually used. In addition,
     * duplicate substrings are removed using a map (implemented with a TernaryTree!).
     */
    ...
    public Enumeration<Object> keys() {
        return (Enumeration<Object>) new com.lowagie.text.pdf.hyphenation.TernaryTree.Iterator();
    }
            ...
            while (climb) {
                ...
                switch (i.child) {
                    case 1:
                        ...
                        ns.push(new com.lowagie.text.pdf.hyphenation.TernaryTree.Iterator.Item(i));  // Use copy constructor
                        climb = false;
                        break;
